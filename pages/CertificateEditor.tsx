...
export const CertificateEditor: React.FC = () => {
  ...
  useEffect(() => {
    if (id) {
      ...
      if (cert) {
        ...
        setDates({
          dob: parseDateForInput(cert.dob),
  ...
  useEffect(() => {
    setFormData(prev => ({
      ...prev,
      dob: formatDateForDisplay(dates.dob),
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    if (id) {
      CertificateService.update(id, formData);
    } else {
      CertificateService.create(formData);
    }
        ...
        <form onSubmit={handleSave} className="bg-white p-6 rounded-lg shadow border border-gray-200 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input type="date" name="dob" value={dates.dob} onChange={handleDateChange} className={inputClassName} />
            </div>
            <div>
              <label className="block text-xs font-semibold text-gray-700 uppercase mb-1">NCQA Number</label>
